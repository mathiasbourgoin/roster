...
| Q | A |
|---|---|
| Does `cwr transpile` exist in cabal-workflow-runner? | No. CWR v0.11.0 has exactly 5 subcommands: `lint`, `validate`, `run`, `replay`, `schema`. The "Claude Code → cwr transpile" dispatch branch from the task brief is invalid. Dispatch simplifies to two branches: (1) `cwr` CLI available → `cwr run`; (2) otherwise → manual skill chain. The Claude Workflow tool integration is deferred to a future CWR release. |
...
| Dispatch priority: CWR vs Claude Code runtime? | `command -v cwr` exits 0 → `cwr run`, regardless of runtime (Claude Code, Codex, etc.). Otherwise → manual chain. No special Claude Code branch until `cwr transpile` is implemented. |
